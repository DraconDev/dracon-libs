...
External/local consumer checks show `dracon-utilities` previously depended on local `../dracon-libs` paths for the used `dracon-git` and `dracon-system-lib` crates and also had unused AI workspace dependencies pointing at local `../dracon-libs` paths. The used dependencies now resolve from `git = "https://github.com/DraconDev/dracon-libs"`, and the unused AI dependencies were removed. `kiki-sassy-desktop-announcer` consumes `dracon-tts-runtime` from crates.io, and `avid` only has a commented-out local STT path.
