...
impl ConfirmDialog {
    ...
    fn render_centered_text(text: &str, width: u16, fg: Color, bg: Color, style: Styles) -> Vec<Cell> {
        ...
        for _ in 0..padding {
            cells.push(Cell { char: ' ', fg, bg, style, transparent: false, skip: false });
        }
        for c in display.chars() {
            cells.push(Cell { char: c, fg, bg, style, transparent: false, skip: false });
        }
    ...
    fn render(&self, area: Rect) -> Plane {
        ...
        for x in 0..area.width {
            ...
            if bottom_idx < plane.cells.len() {
                plane.cells[bottom_idx] = Cell { char: '─', fg: border_fg, bg: self.theme.bg, style: Styles::empty(), transparent: false, skip: false };
            }
        }
        for y in 1..area.height.saturating_sub(1) {
            let left_idx = (y as usize) * area.width as usize;
            plane.cells[left_idx] = Cell { char: '│', fg: border_fg, bg: self.theme.bg, style: Styles::empty(), transparent: false, skip: false };
        ...
        let title_cells = Self::render_centered_text(&self.title, area.width, self.theme.fg, self.theme.bg, Styles::BOLD);
        for (i, cell) in title_cells.into_iter().enumerate().take(area.width as usize) {
            plane.cells[area.width as usize + i] = cell;
        }

        let msg_cells = Self::render_centered_text(&self.message, area.width, self.theme.fg, self.theme.bg, Styles::empty());
        ...
        for (i, c) in confirm_str.chars().enumerate().take(area.width as usize) {
            ...
            if idx < plane.cells.len() {
                plane.cells[idx] = Cell { char: c, fg: if self.danger && self.focused { self.theme.error_fg } else { btn_fg }, bg: self.theme.bg, style: if self.danger { Styles::BOLD } else { Styles::empty() }, transparent: false, skip: false };
            }
        }
        ...
        for (i, c) in cancel_str.chars().enumerate().take(area.width as usize) {
            ...
            if idx < plane.cells.len() {
                plane.cells[idx] = Cell { char: c, fg: btn_fg, bg: self.theme.bg, style: Styles::empty(), transparent: false, skip: false };
            }
        }
