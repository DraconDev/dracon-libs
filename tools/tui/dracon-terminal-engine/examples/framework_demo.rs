...
fn main() -> std::io::Result<()> {
    ...
    App::new()
        ...
        .run(|ctx| {
            ...
            let (breadcrumbs, bread_zones) = {
                let bc = Breadcrumbs::new(vec!["home", "user", "projects", "app"]);
                bc.render(right_rect)
            };
            ctx.add_plane(breadcrumbs);
            for zone in bread_zones {
            ...
            let mut y = 2u16;
            let line = |plane: &mut Plane, text: &str, fg: Color| {
            ...
            line(&mut info_plane, &format!("CPU: {:.1}%", data.cpu_usage), Color::Rgb(0, 200, 120));
            line(&mut info_plane, &format!("Memory: {:.1} / {:.1} GB", data.mem_usage, data.total_mem), Color::Rgb(100, 180, 255));
            line(&mut info_plane, &format!("Swap: {:.1} / {:.1} GB", data.swap_usage, data.total_swap), Color::Rgb(180, 180, 200));
            line(&mut info_plane, &format!("Uptime: {}s", data.uptime), Color::Rgb(150, 150, 150));
            line(&mut info_plane, "", Color::Reset);
            ...
            if let Some(disk) = data.disks.first() {
                ...
                line(&mut info_plane, &format!("Disk: {} ({}%)", disk.name, pct), Color::Rgb(255, 180, 100));
            }
