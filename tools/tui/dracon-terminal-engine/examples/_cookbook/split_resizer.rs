...
//! | Key | Action |
//! |-----|--------|
//! | Click divider | Select (highlighted yellow) |
...
impl Widget for SplitResizerApp {
    ...
    fn handle_key(&mut self, key: dracon_terminal_engine::input::event::KeyEvent) -> bool {
        ...
        match key.code {
            KeyCode::Char('r') => { self.ratio_a = DEFAULT_A; self.ratio_b = DEFAULT_B; self.dirty = true; true }
            KeyCode::Left => { self.ratio_a = (self.ratio_a - 0.05).max(0.1); self.dirty = true; true }
            KeyCode::Right => { self.ratio_a = (self.ratio_a + 0.05).min(0.9); self.dirty = true; true }
            KeyCode::Up | KeyCode::Down => {
                if self.selected.is_none() { self.selected = Some(1); }
    ...
    fn handle_mouse(&mut self, kind: MouseEventKind, col: u16, row: u16) -> bool {
        ...
        match kind {
            MouseEventKind::Down(ratatui::mouse::MouseButton::Left) => {
                if self.in_rect(col, row, div_a) { self.selected = Some(0); self.dirty = true; true }
                else if self.in_rect(col, row, div_b) { self.selected = Some(1); self.dirty = true; true }
                else { false }
            }
            MouseEventKind::Drag(_) => {
                match self.selected {
                    Some(0) => { self.ratio_a = (col as f32 / content.width.max(1) as f32).clamp(0.1, 0.9); self.dirty = true; true }
                    Some(1) => { self.ratio_b = ((row as f32 - 1.0) / content.height.max(1) as f32).clamp(0.1, 0.9); self.dirty = true; true }
                    None => false,
                }
            }
            _ => false,
...
impl SplitResizerApp {
    fn render_header(&self, rect: Rect, p: &mut Plane) {
        ...
        for (i, c) in title.chars().enumerate().take(rect.width as usize) {
            let idx = i;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: c, fg: Color::Rgb(0, 255, 136), bg: Color::Ansi(33), style: Styles::BOLD, transparent: false, skip: false }; }
        }
        for x in 0..rect.width {
            let idx = (rect.width + x) as usize;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: '─', fg: Color::Rgb(80, 80, 100), bg: Color::Ansi(33), style: Styles::empty(), transparent: false, skip: false }; }
        }
    }
    ...
    fn render_content(&self, rect: Rect, p: &mut Plane) {
        ...
        let panel_b2 = Rect::new(panel_b.x, panel_b.y + h1, panel_b.width, panel_b.height - h1);
    ...
    fn render_panel_a(&self, rect: Rect, p: &mut Plane) {
        ...
        self.write_text(rect, p, 0, " Panel A ", Color::Rgb(100, 200, 255), Styles::BOLD);
        ...
        for (i, item) in items.iter().enumerate() {
            ...
            let fg = if i == sel { Color::Rgb(255, 255, 0) } else { Color::Rgb(180, 180, 180) };
            self.write_text(rect, p, (2 + i) as u16, &format!("{}{}", prefix, item), fg, Styles::empty());
        }
    }

    fn render_panel_b1(&self, rect: Rect, p: &mut Plane) {
        self.fill_bg(rect, p, Color::Ansi(17));
        self.write_text(rect, p, 0, " Panel B1 ", Color::Rgb(255, 100, 100), Styles::BOLD);
        ...
        for (i, c) in label.chars().enumerate() {
            ...
            if idx < p.cells.len() { p.cells[idx] = Cell { char: c, fg: Color::Rgb(255, 255, 255), bg: Color::Ansi(17), style: Styles::BOLD, transparent: false, skip: false }; }
        }
        ...
        for i in 0..bar_w {
            ...
            if idx < p.cells.len() { p.cells[idx] = Cell { char: ch, fg, bg: Color::Ansi(17), style: Styles::empty(), transparent: false, skip: false }; }
        }
    }

    fn render_panel_b2(&self, rect: Rect, p: &mut Plane) {
        self.fill_bg(rect, p, Color::Ansi(17));
        self.write_text(rect, p, 0, " Panel B2 ", Color::Rgb(255, 200, 100), Styles::BOLD);
        let lx = rect.x + (rect.width.saturating_sub(self.b2_text.len() as u16)) / 2;
        let ly = rect.y + rect.height / 2;
        for (i, c) in self.b2_text.chars().enumerate() {
            let idx = (ly * p.width + lx + i as u16) as usize;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: c, fg: Color::Rgb(200, 200, 200), bg: Color::Ansi(17), style: Styles::empty(), transparent: false, skip: false }; }
        }
    }

    fn render_divider_h(&self, rect: Rect, w1: u16, p: &mut Plane) {
        let fg = if self.selected == Some(0) { Color::Rgb(255, 255, 0) } else { Color::Rgb(80, 80, 100) };
        for y in 0..rect.height {
            let idx = ((rect.y + y) * p.width + rect.x + w1) as usize;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: if y == rect.height / 2 { '╮' } else { '│' }, fg, bg: Color::Ansi(33), style: Styles::empty(), transparent: false, skip: false }; }
        }
    }

    fn render_divider_v(&self, rect: Rect, h1: u16, p: &mut Plane) {
        let fg = if self.selected == Some(1) { Color::Rgb(255, 255, 0) } else { Color::Rgb(80, 80, 100) };
        for x in 0..rect.width {
            let idx = ((rect.y + h1) * p.width + rect.x + x) as usize;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: if x == rect.width / 2 { '┴' } else { '─' }, fg, bg: Color::Ansi(33), style: Styles::empty(), transparent: false, skip: false }; }
        }
    }
    ...
    fn render_status(&self, rect: Rect, p: &mut Plane) {
        ...
        let sb = format!("Panel B: {}%", ((1.0 - self.ratio_a) * 100.0).round() as i32);
        let sb1 = format!("Panel B1: {}%", (self.ratio_b * 100.0).round() as i32);
        let text = format!("{} | {} | {} | ←/→:A  ↑/↓:B  r:reset", sa, sb, sb1);
        for (i, c) in text.chars().enumerate().take(rect.width as usize) {
            let idx = (rect.y * p.width + i as u16) as usize;
            if idx < p.cells.len() { p.cells[idx] = Cell { char: c, fg: if i < sa.len() + sb.len() + 4 { Color::Rgb(0, 255, 136) } else { Color::Rgb(150, 150, 150) }, bg: Color::Ansi(17), style: Styles::BOLD, transparent: false, skip: false }; }
        }
    }
    ...
    fn write_text(&self, rect: Rect, p: &mut Plane, y: u16, text: &str, fg: Color, style: Styles) {
        for (i, c) in text.chars().take(rect.width as usize).enumerate() {
            ...
            if idx < p.cells.len() { p.cells[idx] = Cell { char: c, fg, bg: Color::Ansi(17), style, transparent: false, skip: false }; }
        }
    }
    ...
    fn fill_bg(&self, rect: Rect, p: &mut Plane, bg: Color) {
        for y in 0..rect.height {
            for x in 0..rect.width {
                ...
                if idx < p.cells.len() { p.cells[idx].bg = bg; }
            }
        }
    }
...
fn main() -> Result<()> {
    ...
    app.on_tick(move |ctx, tick| {
        if tick % 8 == 0 { app.b1_pct = rand::thread_rng().gen_range(10.0..90.0); }
        if tick % 12 == 0 {
            let texts = ["Status: Active", "Status: Processing", "Status: Idle", "Status: Waiting"];
            app.b2_text = texts[rand::thread_rng().gen_range(0..texts.len())].to_string();
        }
        app.mark_dirty();
    }).run(move |ctx| {
        let (w, h) = ctx.compositor().size();
        if app.area.get().width != w || app.area.get().height != h { app.set_area(Rect::new(0, 0, w, h)); }
