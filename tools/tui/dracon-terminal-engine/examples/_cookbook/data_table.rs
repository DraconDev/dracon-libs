...
impl Sort {
    fn next(&self) -> Self {
        match self { Sort::None => Sort::Asc, Sort::Asc => Sort::Desc, Sort::Desc => Sort::None }
    }
    fn sym(&self) -> &'static str {
        match self { Sort::None => "", Sort::Asc => "▲", Sort::Desc => "▼" }
    }
}
...
impl Table {
    ...
    fn filter(&mut self, q: &str) {
        self.rows = if q.is_empty() { self.all.clone() }
            else { self.all.iter().filter(|p| p.matches(q)).cloned().collect() };
        self.sort_rows();
        self.sel = 0;
        self.off = 0;
        self.dirty = true;
...
impl Widget for Table {
    ...
    fn render(&self, area: Rect) -> Plane {
        ...
        p.z_index = 10;
        let heads = ["Name", "Age", "City", "Profession"];
        ...
        for x in 0..area.width {
            ...
            if idx < p.cells.len() { p.cells[idx].char = '─'; p.cells[idx].fg = Color::Ansi(240); }
        }
        ...
        match key.code {
            KeyCode::Down => {
                if self.sel + 1 < self.rows.len() {
                    self.sel += 1;
                    if self.sel >= self.off + self.vis { self.off = self.sel - self.vis + 1; }
                    self.dirty = true;
                }
                true
            }
            KeyCode::Up => {
                if self.sel > 0 { self.sel -= 1; if self.sel < self.off { self.off = self.sel; } self.dirty = true; }
                true
            }
        ...
        match kind {
            crate::input::event::MouseEventKind::Down(crate::input::event::MouseButton::Left) => {
                if row == hh - 1 && col >= self.area.width.saturating_sub(14) {
                    self.sort = self.sort.next(); self.sort_rows(); self.dirty = true; true
                } else if row >= hh && row < self.area.height - sh {
                    let idx = self.off + (row - hh) as usize;
                    if idx < self.rows.len() { self.sel = idx; self.dirty = true; true } else { false }
                } else { false }
            }
